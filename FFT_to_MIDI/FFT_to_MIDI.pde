...
void setup()
{
  size(legend_width+spectrum_width+20, spectrum_height+legend_height+30,P2D);
...
void draw(){
  ...
  // draw peak bars
  stroke(0);
  fill(50, 200, 255); //color bar
  for(int i = 0; i < peaksize; ++i) { 

    int thisy = spectrum_height - Math.round(peaks[i]);
        int alto = spectrum_height-thisy;
    rect(legend_width+binsperband*i, thisy, binsperband, alto);
    // update decays
